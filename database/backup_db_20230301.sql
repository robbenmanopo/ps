...
-- Dumping data for table ps.data_guru: ~3 rows (approximately)
...
-- Dumping data for table ps.kd: ~4 rows (approximately)
...
-- Dumping data for table ps.mapel: ~6 rows (approximately)
...
-- Dumping data for table ps.role: ~3 rows (approximately)
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `view_kd_input` AS select `kd`.`id_mapel` AS `id_mapel`,`kd`.`id_kd` AS `id_kd`,`kd`.`kode_kd` AS `kode_kd`,`kd`.`deskripsi` AS `deskripsi`,`mapel`.`nama_mapel` AS `nama_mapel`,`mapel`.`id_kelas` AS `id_kelas`,`data_guru`.`id_guru` AS `id_guru`,`data_guru`.`nik` AS `nik`,`data_guru`.`jk` AS `jk`,`data_guru`.`nama_guru` AS `nama_guru`,`data_guru`.`username` AS `username`,`data_guru`.`password` AS `password`,`data_guru`.`id_role` AS `id_role` from ((`kd` join `mapel` on((`kd`.`id_mapel` = `mapel`.`id_mapel`))) join `data_guru` on((`kd`.`id_mapel` = `data_guru`.`id_mapel`)));
